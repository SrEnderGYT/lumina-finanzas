...
export const bcpParser: BankParser = {
  ...
  parse: (email) => {
    const parsed = parseCommon(email, "BCP", "bcp-v1", [
    ...
    if (parsed && /adelanto de sueldo/i.test(`${email.subject} ${email.body}`)) return { ...parsed, merchant: "Adelanto de sueldo BCP", operationType: "income" };
    return parsed;
  },
};
