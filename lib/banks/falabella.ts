...
export const falabellaParser: BankParser = {
  ...
  parse: (email) => parseCommon(email, "Banco Falabella", "falabella-v1", [/\b(?:comercio|establecimiento|compra en|consumo en)\s*[:\-]?\s*([^|]{3,70}?)(?=\s+(?:por|monto|importe|el día|con (?:tu|tarjeta))|[.;])/i], .95),
};
