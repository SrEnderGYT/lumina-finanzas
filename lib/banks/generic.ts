...
export const genericParser: BankParser = {
  ...
  parse: (email) => parseCommon(email, "Otro banco", "generic-v1", [/\b(?:comercio|establecimiento|compra en|consumo en|pago en)\s*[:\-]?\s*(?!\d)([^|]{3,70}?)(?=\s+(?:por|monto|importe|el día|con (?:tu|tarjeta))|[.;])/i], .65),
};
